...
void stopTimer(char rw, char iotype, char testcase, char *filename) {
   ...
   const char *str_rw = (rw == 'r')   ? "Reads : "
                        : (rw == 'w') ? "Writes: "
                                      : "Unknown";
   const char *str_iotype = (iotype == 'u')   ? "Unix   I/O"
                            : (iotype == 'f') ? "C File I/O"
                                              : "Unknown";
   const char *str_testcase = (testcase == 'a')   ? "Read   once     "
                              : (testcase == 'b') ? "Block  transfers"
                              : (testcase == 'c') ? "Char   transfers"
                              : (testcase == 'r') ? "Random transfers"
                                                  : "Unknown";
...
void writes(char iotype, char testcase, char *filename) {
   int fd = (iotype == 'u') ? open(filename, O_WRONLY | O_CREAT | O_TRUNC,
